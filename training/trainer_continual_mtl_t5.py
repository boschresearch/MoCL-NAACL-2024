...
class ContinualTrainerMTL(nn.Module):
    ...
    def compute_loss(self, model, inputs, task_id, mode, final=False, prompt_select_mode='local_compose', id_pred_on=False):
        train = True if mode == 'train' else False
        loss, outputs = model(inputs, task_id, train, final, prompt_select_mode, id_pred_on)
            ...
            for task_id, task in enumerate(self.task_list):
                ...
                for epoch in range(self.num_train_epochs):
                    ...
                    for batch_index in tqdm(range(num_train_batch), total=num_training_steps):
                        ...
                        train_loss, _ = self.compute_loss(self.model, train_input, task_id, mode='train', prompt_select_mode=prompt_select_mode)
                    ...
                    best_epoch = epoch
                    if epoch > 0 and self.early_stopping_patience > 0 and (self.early_stopping(eval_acc_, self.model, task) or epoch == self.num_train_epochs-1):
                        ...
                        if self.model.config.multi_peft_modules and self.model.config.compose_prompts:
                            self.logger.info(f"Weight distribution up to the {task_id}-th task: {self.model.prefix_encoder.attn_weights[task_id]}")
    ...
    def eval(self, loader, batch, task_id, mode='test', final=False, prompt_select_mode='local_compose', id_pred_on=False):
        # Notes regarding 'prompt_select_mode'
        # 1. validation is always: prompt_select_mode='local_compose'; (except for the warm_up stage, also the same in training)
        # 2. TIL (3 options): prompt_select_mode='task_id'/'local_compose'/'global_compose'
        # 3. CIL (3 options): prompt_select_mode='task_id'/'local_compose'/'global_compose'; 
        ...
        with torch.no_grad():
            for batch_index in tqdm(range(batch)):
                ...
                _, outputs = self.compute_loss(self.model, input, task_id, mode=mode, final=final, prompt_select_mode=prompt_select_mode, id_pred_on=id_pred_on)
                
                dec = [self.tokenizer.decode(ids) for ids in outputs]
                targets = [self.tokenizer.decode(ids) for ids in gt]
