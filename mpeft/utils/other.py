...
class ModulesToSaveWrapper(torch.nn.Module):
    ...
    def update(self, adapter_name, reinit_cls_module=None):
        module_update = reinit_cls_module if reinit_cls_module else copy.deepcopy(self.original_module)
        self.modules_to_save.update(torch.nn.ModuleDict({adapter_name: module_update}))
...
def _set_trainable(model, adapter_name):
    ...
    for key in key_list:
        ...
        if target_module_found:
            parent, target, target_name = _get_submodules(model, key) # C: parent - the llama model; target - the classifier; target_name - 'score'
