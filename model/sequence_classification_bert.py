...
class BertPrefixContinualForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        prefix_attention_mask = torch.ones(batch_size, past_key_values[0].shape[-2]).to(self.bert.device)
        ...
        if self.config.task_specific_classifier:
            ...
            if train:
                ...
                if classifier_select_mode == 'task_id':
                    ...
                    logits = (torch.ones(batch_size, self.config.num_labels).to(self.bert.device)) *(-float('inf'))
